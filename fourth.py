def je_tah_mozny_pesec(cilova_pozice, aktualni_pozice, obsazene_pozice):
    # pokud se jedna o jiny sloupec, vrat False
    if aktualni_pozice[1] != cilova_pozice[1]:
        return False
    # pokud se pesak posune o 1 pole dopredu, vrat True
    if aktualni_pozice[0] + 1 == cilova_pozice[0]:
        return True
    # pokud je pesak na radku 2 a posune se o 2 pole dopredu a neni obsazeno pole po ceste, vrat True
...
def je_tah_mozny_strelec(cilova_pozice, aktualni_pozice, obsazene_pozice):
    ...
    for i in range(1, abs(aktualni_pozice[0] - cilova_pozice[0])):
        if (cilova_pozice[0]>aktualni_pozice[0]) and (cilova_pozice[1]>aktualni_pozice[1]) and (aktualni_pozice[0] + i, aktualni_pozice[1] + i) in obsazene_pozice:
            return False
        elif (cilova_pozice[0]>aktualni_pozice[0]) and (cilova_pozice[1]<aktualni_pozice[1]) and (aktualni_pozice[0] + i, aktualni_pozice[1] - i) in obsazene_pozice:
            return False
        elif (cilova_pozice[0]<aktualni_pozice[0]) and (cilova_pozice[1]>aktualni_pozice[1]) and (aktualni_pozice[0] - i, aktualni_pozice[1] + i) in obsazene_pozice:
            return False
        elif (cilova_pozice[0]<aktualni_pozice[0]) and (cilova_pozice[1]<aktualni_pozice[1]) and (aktualni_pozice[0] - i, aktualni_pozice[1] - i) in obsazene_pozice:
...
def je_tah_mozny(figurka, cilova_pozice, obsazene_pozice):
    ...
    if cilova_pozice in obsazene_pozice:
        return False
    if cilova_pozice[0]>8 or cilova_pozice[0]<1 or cilova_pozice[1]>8 or cilova_pozice[1]<1:
        return False
...
if __name__ == "__main__":
    ...
    print()
    print(je_tah_mozny(strelec, (1, 8), obsazene_pozice))  # Nevím proč by to mělo být True, střelec je blokován políčkem (5,4)